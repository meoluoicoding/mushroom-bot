...
impl CordycepsProtocol {
    pub fn new() -> Self {
        Self { board: None, my_player: FIRST, data: CordycepsData::load() }
    }
        ...
        for line in stdin.lock().lines() {
            ...
            match parts[0] {
                ...
                "TIME" => {
                    ...
                    let my_ms: i32 = parts.get(1).and_then(|s| s.parse().ok()).unwrap_or(0);
                    let opp_ms: i32 = parts.get(2).and_then(|s| s.parse().ok()).unwrap_or(0);
                    let (r1, c1, r2, c2) = hybrid_search(&state, my_ms, opp_ms, &self.data);
